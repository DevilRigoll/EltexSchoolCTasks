...
char ** getStringsArray(char * text, int * size) {
	...
	//int ssize = 1;
	/*
	arr_str[0] = (char *)malloc((strlen(t) + 1) * sizeof(char));
	strcpy(arr_str[0], t);
	...
	while (1) {
		...
		if (t == NULL)
			break;
		ssize++;
		arr_str = (char **)realloc(arr_str, ssize * sizeof(char *));
		arr_str[ssize - 1] = (char *)malloc((strlen(t) + 1) * sizeof(char));
		strcpy(arr_str[ssize - 1], t);
	}
